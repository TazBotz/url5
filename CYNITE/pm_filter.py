...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⟸ Gᴏ ᴛᴏ Hᴏᴍᴇ', callback_data='start'),
        ...
        await query.message.edit_text(
            ...
            cnlbtn = [[
                      InlineKeyboardButton('Cʜᴀɴɴᴇʟ', url='t.me/TazHD')
                     ], [
                      InlineKeyboardButton('Sᴜᴘᴘᴏʀᴛ', url='t.me/XKunalBot'),
                      InlineKeyboardButton('Uᴘᴅᴀᴛᴇs', url='t.me/TazHD')
                     ], [
        ...
        await query.edit_message_text(text=f"<s>{query.message.text}</s>",
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="⚠️ Uɴᴀᴠᴀɪʟᴀʙʟᴇ ⚠️", callback_data="notavl")],[InlineKeyboardButton(text="Rᴇǫᴜᴇsᴛ Aɢᴀɪɴ", url=GRP_LNK)]])
        )
        ...
        await query.edit_message_text(text=f"<s>{query.message.text}</s>",
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="❌ Rᴇᴊᴇᴄᴛᴇᴅ ❌", callback_data="rjctreq")],[InlineKeyboardButton(text="Rᴇǫᴜᴇsᴛ Aɢᴀɪɴ", url=GRP_LNK)]])
        )
        ...
        await query.edit_message_text(text=f"<s>{query.message.text}</s>",
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="♻️ Aʟʀᴇᴀᴅʏ Aᴠᴀɪʟᴀʙʟᴇ ♻️", callback_data="donealrd")],[InlineKeyboardButton(text="Rᴇǫᴜᴇsᴛ Aɢᴀɪɴ", url=GRP_LNK)]])
        )
        await client.send_message(SUPPORT_GROUP, text=script.DONE_ALREADY2,
            reply_to_message_id=int(mess_id),
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="🔍Pʟᴇᴀꜱᴇ Sᴇᴀʀᴄʜ Hᴇʀᴇ🔍", url=f"https://t.me/+AJCnlql9y4o4MjJl")],[InlineKeyboardButton(text="♻️Cʜᴇᴄᴋ Yᴏᴜʀ Rᴇǫᴜᴇsᴛ♻️", url=f"{query.message.link}")]]),
        )
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            if not files:
                await client.send_message(req_channel,f"-🦋 #REQUESTED_CONTENT 🦋-\n\n📝Content Name :{search}\nRequested By: {message.from_user.first_name}\n USER ID:{message.from_user.id}\n\n🗃", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔺 Mark as Done 🔺", callback_data=f"req_aval {mess_id} {bmess_id}")]]))
    ...
    if settings["button"] and msg.chat.id not in filters.chat(chats=SUPPORT_CHAT):
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {file.file_name}",
...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        btn = [[
            InlineKeyboardButton('Sᴇɴᴅ Rᴇǫᴜᴇsᴛ', callback_data='rqstfrmt'),
            InlineKeyboardButton('ɪɴꜱᴛᴜʀᴄᴛɪᴏɴs', callback_data='splmd')
        ],[
            InlineKeyboardButton('🔍Cʜᴇᴄᴋ ꜱᴘᴇʟʟɪɴɢ ɪɴ Gᴏᴏɢʟᴇ🔍', url=f"https://google.com/search?q={search}")
        ]]           
